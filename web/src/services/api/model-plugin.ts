...
function rawPluginUrl(config: AiConfig, path: string) {
    if (/^https?:/i.test(path)) return path;
    return `${config.baseUrl.trim().replace(/\/+$/, "")}/${path.replace(/^\/+/, "")}`;
}
...
function createPluginRequest(config: AiConfig, options?: RequestOptions) {
    return async (requestConfig: AxiosRequestConfig & { url: string }) => {
        const response = await axios.request({ ...requestConfig, url: rawPluginUrl(config, requestConfig.url), signal: options?.signal });
        return response.data;
    };
}
