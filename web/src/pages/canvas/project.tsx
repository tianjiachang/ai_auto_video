        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                ...
                if (mode === "video") {
                    ...
                    const h3Settings = {
                        h3GenerationMode: generationConfig.h3GenerationMode,
                        h3InputMode: generationConfig.h3InputMode,
                        h3SourceTaskId: generationConfig.h3SourceTaskId,
                        h3ContextIrEnabled: generationConfig.h3ContextIrEnabled,
                    };
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, { images: generationContext.referenceImages, videos: generationContext.referenceVideos, audios: generationContext.referenceAudios }, { signal: controller.signal }),
                        );
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    const selfVideo: ReferenceVideo[] = node.metadata?.content
                        ? [{ id: node.id, name: `${node.title || node.id}.mp4`, type: node.metadata.mimeType || "video/mp4", url: node.metadata.content, storageKey: node.metadata.storageKey, bytes: node.metadata.bytes, width: node.metadata.naturalWidth, height: node.metadata.naturalHeight, durationMs: node.metadata.durationMs, providerTaskId: node.metadata.providerTaskId }]
                        : [];
                    const video = await storeGeneratedVideo(
                        await requestVideoGeneration(
                            generationConfig,
                            prompt,
                            { images: retryImages, videos: [...selfVideo, ...(context?.referenceVideos || [])], audios: context?.referenceAudios || [] },
                            { signal: controller.signal },
                        ),
                    );
