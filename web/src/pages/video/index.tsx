...
import { MetasoH3Settings } from "@/components/metaso-h3-settings";
...
import { createVideoGenerationTask, pollVideoGenerationTask, storeGeneratedVideo, type VideoGenerationInput, type VideoGenerationTask } from "@/services/api/video";
import { useAssetStore } from "@/stores/use-asset-store";
import { useWorkbenchAgentStore } from "@/stores/use-workbench-agent-store";
import { boolConfig, isMetasoH3VideoModel, modelOptionLabel, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark" | "h3GenerationMode" | "h3InputMode" | "h3SourceTaskId" | "h3ContextIrEnabled">;
...
export default function VideoPage() {
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        const unsupported = selectedFiles.filter((file) => !file.type.startsWith("image/") && !file.type.startsWith("video/") && !file.type.startsWith("audio/"));
        if (unsupported.length) message.warning(t("videoWorkbench.unsupportedFiles"));
        const imageFiles = selectedFiles.filter((file) => file.type.startsWith("image/") && file.size <= 30 * 1024 * 1024).slice(0, 9 - references.length);
        const videoFiles = selectedFiles.filter((file) => file.type.startsWith("video/") && file.size <= 200 * 1024 * 1024).slice(0, 3 - referenceVideos.length);
        const audioFiles = selectedFiles.filter((file) => file.type.startsWith("audio/") && file.size <= 15 * 1024 * 1024).slice(0, 3 - referenceAudios.length);
        ...
        const [nextVideos, nextAudios] = await Promise.all([
    ...
    const generate = async () => {
        ...
        try {
            ...
            const log = buildLog({ prompt: snapshot.text, model, config: snapshot.config, input: snapshot.input, durationMs: 0, status: "pending", task });
            ...
            await saveLog(buildLog({ prompt: snapshot.text, model, config: snapshot.config, input: snapshot.input, durationMs: performance.now() - batchStartedAt, status: "failed", error: errorMessage }));
    ...
    const buildRequestSnapshot = () => {
        ...
        return {
            ...
            input: { images: [...references], videos: [...referenceVideos], audios: [...referenceAudios] },
        };
    };
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        if (payload.kind === "text") {
            ...
            setReferenceVideos((value) => [...value, { id: nanoid(), name: payload.title, type: "video/mp4", url: payload.url, storageKey: payload.storageKey, width: payload.width, height: payload.height }].slice(0, 3));
        }
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig, agentTaskId?: string) => {
        ...
        try {
            const maxAttempts = log.task.provider === "metaso-h3" ? 240 : 120;
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        if (log.config.h3GenerationMode) updateConfig("h3GenerationMode", log.config.h3GenerationMode);
        if (log.config.h3InputMode) updateConfig("h3InputMode", log.config.h3InputMode);
        if (log.config.h3SourceTaskId) updateConfig("h3SourceTaskId", log.config.h3SourceTaskId);
        if (log.config.h3ContextIrEnabled) updateConfig("h3ContextIrEnabled", log.config.h3ContextIrEnabled);
                                ...
                                >
                                    ...
                                    {referenceVideos.map((item, index) => (
                                        <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 bg-black dark:border-stone-800">
                                            ...
                                            <span className="absolute left-1 top-1 rounded bg-black/60 px-1.5 py-0.5 text-[10px] font-medium text-white">V{index + 1}</span>
                                    ...
                                    {referenceAudios.map((item, index) => (
                                        <div key={item.id} className="group relative flex size-20 shrink-0 flex-col items-center justify-center overflow-hidden rounded-md border border-stone-200 bg-stone-100 text-stone-500 dark:border-stone-800 dark:bg-stone-900 dark:text-stone-400">
                                            <Music2 className="size-6" />
                                            <span className="mt-1 w-full truncate px-1 text-center text-[10px]">A{index + 1}</span>
                                    ...
                                    {!references.length && !referenceVideos.length && !referenceAudios.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceDragTarget ? t("videoWorkbench.dropReferences") : t("videoWorkbench.noImages")}</div> : null}
                                </div>
                            </div>
...
function GenerationSettings({ config, model, updateConfig, openConfigDialog }: { config: AiConfig; model: string; updateConfig: UpdateAiConfig; openConfigDialog: (shouldPromptContinue?: boolean) => void }) {
    ...
    const { t } = useTranslation();
    const isMetasoH3 = isMetasoH3VideoModel(config, model);
    ...
    return (
        <>
            ...
            {isMetasoH3 ? <div className="col-span-2"><MetasoH3Settings config={config} onConfigChange={updateConfig} /></div> : null}
        </>
    );
}
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        h3GenerationMode: log.config?.h3GenerationMode || "generate",
        h3InputMode: log.config?.h3InputMode || "auto",
        h3SourceTaskId: log.config?.h3SourceTaskId || "",
        h3ContextIrEnabled: log.config?.h3ContextIrEnabled || "true",
    };
}

function buildLog({ prompt, model, config, input, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; input: VideoGenerationInput; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    const logConfig = {
        model: config.model,
        ...
        h3GenerationMode: config.h3GenerationMode,
        h3InputMode: config.h3InputMode,
        h3SourceTaskId: config.h3SourceTaskId,
        h3ContextIrEnabled: config.h3ContextIrEnabled,
    };
    return {
        id: nanoid(),
        createdAt: Date.now(),
        ...
        referenceVideos: input.videos || [],
        referenceAudios: input.audios || [],
        durationMs,
        size: logConfig.size,
