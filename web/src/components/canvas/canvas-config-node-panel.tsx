...
import { defaultConfig, isMetasoH3VideoModel, resolveModelForCapability, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const isMetasoH3 = mode === "video" && isMetasoH3VideoModel(globalConfig, config.model);
            ...
            {isMetasoH3 ? (
                <div className="mb-2 flex justify-end">
                    <Popover
                        trigger="click"
                        placement="topRight"
                        content={<div className="w-72"><MetasoH3Settings compact config={config} onConfigChange={(key, value) => onConfigChange(node.id, { [key]: value } as Partial<CanvasNodeMetadata>)} /></div>}
                    >
                        <Tooltip title={t("videoWorkbench.h3.title")}>
                            <Button size="small" type="text" className="!h-7 !w-7 !min-w-7 !p-0" icon={<Settings2 className="size-3.5" />} aria-label={t("videoWorkbench.h3.title")} />
                        </Tooltip>
                    </Popover>
                </div>
            ) : null}
...
function buildNodeConfig(globalConfig: AiConfig, node: CanvasNodeData, mode: CanvasGenerationMode): AiConfig {
    return {
        ...
        h3GenerationMode: node.metadata?.h3GenerationMode || globalConfig.h3GenerationMode || defaultConfig.h3GenerationMode,
        h3InputMode: node.metadata?.h3InputMode || globalConfig.h3InputMode || defaultConfig.h3InputMode,
        h3SourceTaskId: node.metadata?.h3SourceTaskId || globalConfig.h3SourceTaskId || defaultConfig.h3SourceTaskId,
        h3ContextIrEnabled: node.metadata?.h3ContextIrEnabled || globalConfig.h3ContextIrEnabled || defaultConfig.h3ContextIrEnabled,
