...
export type H3GenerationMode = "generate" | "regenerate-task" | "regenerate-video";
export type H3InputMode = "auto" | "frames" | "references";
...
export type AiConfig = {
    ...
    h3SourceTaskId: string;
    h3ContextIrEnabled: string;
    systemPrompt: string;
...
export const defaultConfig: AiConfig = {
    ...
    h3GenerationMode: "generate",
    h3InputMode: "auto",
    h3SourceTaskId: "",
    h3ContextIrEnabled: "true",
    systemPrompt: "",
            ...
            merge: (persisted, current) => {
                ...
                return {
                    ...
                    config: {
                        ...
                        h3GenerationMode: normalizeH3GenerationMode(config.h3GenerationMode),
                        h3InputMode: normalizeH3InputMode(config.h3InputMode),
                        h3SourceTaskId: config.h3SourceTaskId || "",
                        h3ContextIrEnabled: config.h3ContextIrEnabled || "true",
...
export function isMetasoH3VideoModel(config: AiConfig, value: string) {
    const channel = resolveModelChannel(config, value);
    const model = modelOptionName(value).toLowerCase();
    return model.startsWith("minimax-h3") && /^https:\/\/metaso\.cn\/api\/minimax\/?$/i.test(channel.baseUrl.trim());
}

export function createMetasoH3Channel() {
...
function normalizeH3GenerationMode(value: unknown): H3GenerationMode {
    return value === "regenerate-task" || value === "regenerate-video" ? value : "generate";
}

function normalizeH3InputMode(value: unknown): H3InputMode {
    return value === "frames" || value === "references" ? value : "auto";
}
